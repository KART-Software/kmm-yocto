...
### 2.2 クロックのはしご — 誰が CPU を速くしていくか

CPU クロックはブートチェーンの各段が一段ずつ引き上げる。**上げる前に必ず
「電圧を先に上げる」「PLL の再設定中は CPU を別クロックに退避する」**のが作法。

| 段階 | A53 クロック | やること |
|---|---|---|
| リセット直後 | **24MHz(水晶直結)** | CCM のリセット既定はソース 0 = 24M osc。ROM の最初の命令はこの速度で走る |
| BootROM | **~800MHz** | ROM が SYS_PLL1(800M)を立てて A53 ルートを切替。既定電圧(ND 0.85V)で安全な速度までしか上げない |
| SPL | **1.2GHz**(8MM/8MP とも) | 先に PMIC で VDD_ARM を 0.95V へ昇圧 → A53 を一旦 SYS_PLL1_800M に**退避** → ARM PLL を 1200MHz に再設定 → MUX を ARM PLL へ戻す(clock_imx8mm.c の clock_init) |
| ATF / U-Boot proper | 変更なし | — |
| カーネル cpufreq | **1.2〜定格 GHz を動的に** | DT の OPP 表 × スピードグレード fuse(個体の定格を fuse から読む)× governor。OPP ごとの電圧もここで連動 |

つまり「ブートローダ〜カーネル初期は 1.2GHz 固定」。起動時間を削るなら
SPL の ARM PLL 値を定格まで上げる手がある(8MM で 1.8GHz 化を実施済み:
kernel→GUI 3.33→2.98s。電圧は SPL が既に overdrive 値へ上げているのが前提。
8MP は 1.6GHz 品、未実施)。ondemand governor はブート中の昇速が遅い点も同根。
