...
class DP100:
    ...
    def apply_set(self, on, vo_V=None, io_A=None, ovp_V=None, ocp_A=None):
        ...
        payload = struct.pack("<BBHHHH", IDX_SAVE_APPLY + cur["index"], state, vo, io, ovp, ocp)
        d = self._txn(OP_BASIC_SET, payload)
        if not d or d[0] != 0x01:
            raise IOError(f"device rejected basic_set (resp={d.hex() if d else 'empty'})")
