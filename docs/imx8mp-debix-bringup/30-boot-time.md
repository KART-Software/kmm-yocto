...
| — | 09-11 | **Cortex-M7 統合**(kas/imx8mp-m7.yml: BL31 が M7 起動、Linux は remoteproc attach、CAN は rpmsg-can の rpmsgcan0。01-m7.md) | **3.35〜3.48s** | **+0.10s** | 対照 = 同ツリーで m7 オーバーレイだけ外した像 3.26s(σ0.05、N=5)。増分はカーネル側: remoteproc attach の `imx_rproc_kick` が M7 の MU 未応答で tx_tout=100ms タイムアウト(err -62)し、root マウント前の `wait_for_device_probe()` に乗って PID1 が 0.76→0.85s に遅れる → 以降(seed/weston/kmm)が丸ごと 0.09s ずれる。can0-up/modules-load をマスクしても不変(userspace 部品は無罪)。fix 候補は M7 側で MU を drain して kick を即完了させる。CAN 自体は rpmsg-can を modules-load.d で早期ロードして UP 1.75s(kmm 起動前)= GUI と同時に値が入る |
