...
- ~~CAN INT のパッド(SAI5_RXD3=GPIO3_IO24)~~ → **解消 (2026-08-13)**:
  HAT 装着状態で CAN フレームの実受信を確認。mcp2515 の受信は割り込み駆動
  のみ (ポーリング経路なし) のため、受信成立 = INT 配線が GPIO3_IO24 で
  正しいことの実証になる。これで bring-up 以来の未検証仮定は全て解消。
