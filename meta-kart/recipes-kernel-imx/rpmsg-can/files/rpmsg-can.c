...
/*
 * rpmsg-can — Cortex-M4 CAN ゲートウェイを SocketCAN の candev として見せる
 *
...
enum {
	...
	RPMSG_CAN_CMD_SET_MODE    = 2,	/* Linux→M4: flags = ctrlmode ビット */
...
#define RPMSG_CAN_MODE_LISTENONLY	BIT(0)
#define RPMSG_CAN_MODE_LOOPBACK		BIT(1)

struct rpmsg_can_priv {
	struct can_priv can;		/* 先頭必須 (netdev_priv → can_priv) */
	struct rpmsg_device *rpdev;
};

/* 実タイミングは M4 側。Linux は許可ビットレートを列挙するだけ */
static const u32 rpmsg_can_bitrates[] = {
	125000, 250000, 500000, 1000000,
};

static int rpmsg_can_ctrl_send(struct rpmsg_can_priv *priv, u8 cmd, u8 flags,
			      u32 arg)
{
	struct rpmsg_can_ctrl c = {
...
static int rpmsg_can_open(struct net_device *ndev)
{
	...
	if (priv->can.ctrlmode & CAN_CTRLMODE_LISTENONLY)
		mode |= RPMSG_CAN_MODE_LISTENONLY;
	if (priv->can.ctrlmode & CAN_CTRLMODE_LOOPBACK)
		mode |= RPMSG_CAN_MODE_LOOPBACK;

	/* M4 へ設定を転送 (最初の制御メッセージで M4 は返信先 ept も学習する)。
	 * sim は制御を無視して streaming を始めるだけなので互換 */
	rpmsg_can_ctrl_send(priv, RPMSG_CAN_CMD_SET_BITRATE, 0,
			   priv->can.bittiming.bitrate);
	rpmsg_can_ctrl_send(priv, RPMSG_CAN_CMD_SET_MODE, mode, 0);
	rpmsg_can_ctrl_send(priv, RPMSG_CAN_CMD_START, 0, 0);
...
static int rpmsg_can_stop(struct net_device *ndev)
{
	struct rpmsg_can_priv *priv = netdev_priv(ndev);

	netif_stop_queue(ndev);
	rpmsg_can_ctrl_send(priv, RPMSG_CAN_CMD_STOP, 0, 0);
...
static netdev_tx_t rpmsg_can_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	struct rpmsg_can_priv *priv = netdev_priv(ndev);
...
static const struct net_device_ops rpmsg_can_netdev_ops = {
	.ndo_open = rpmsg_can_open,
	.ndo_stop = rpmsg_can_stop,
	.ndo_start_xmit = rpmsg_can_xmit,
};

/* M4 からのデータフレーム (16B) を SocketCAN へ注入 */
static void rpmsg_can_rx_frame(struct net_device *ndev,
			      const struct rpmsg_can_wire *w)
{
	struct can_frame *cf;
	struct sk_buff *skb;
...
static void rpmsg_can_rx_ctrl(struct net_device *ndev,
			     const struct rpmsg_can_ctrl *c)
{
	struct rpmsg_can_priv *priv = netdev_priv(ndev);
	...
	if (c->cmd == RPMSG_CAN_EVT_STATE && arg <= CAN_STATE_BUS_OFF) {
			     ...
			     void *priv, u32 src)
{
	...
	if (len == sizeof(struct rpmsg_can_wire))
		rpmsg_can_rx_frame(ndev, data);
	else if (len == sizeof(struct rpmsg_can_ctrl))
		rpmsg_can_rx_ctrl(ndev, data);
	/* それ以外 (ハンドシェイク等) は無視 */
	return 0;
}
...
static int rpmsg_can_probe(struct rpmsg_device *rpdev)
{
	...
	priv->can.bitrate_const = rpmsg_can_bitrates;
	priv->can.bitrate_const_cnt = ARRAY_SIZE(rpmsg_can_bitrates);
	priv->can.ctrlmode_supported =
		CAN_CTRLMODE_LISTENONLY | CAN_CTRLMODE_LOOPBACK;
	priv->can.do_set_mode = rpmsg_can_set_mode;
	/* 明示 bitrate 無しの `ip link up` でも上がるようデフォルトを入れる */
	priv->can.bittiming.bitrate = 500000;

	ndev->netdev_ops = &rpmsg_can_netdev_ops;
	...
	netdev_info(ndev, "rpmsg-can channel bound (remote ept 0x%x)\n",
		    rpdev->dst);
	return 0;
}
...
static const struct rpmsg_device_id rpmsg_can_id_table[] = {
	{ .name = "rpmsg-can" },
	{},
};
MODULE_DEVICE_TABLE(rpmsg, rpmsg_can_id_table);

static struct rpmsg_driver rpmsg_can_driver = {
	.probe = rpmsg_can_probe,
	.remove = rpmsg_can_remove,
	.callback = rpmsg_can_cb,
	.id_table = rpmsg_can_id_table,
	.drv = {
		.name = "rpmsg-can",
	},
};
module_rpmsg_driver(rpmsg_can_driver);

MODULE_DESCRIPTION("CAN candev backed by rpmsg (Cortex-M4 gateway)");
MODULE_LICENSE("GPL");
