...
# prebuilt 同梱。FALCON_M4 のとき m4-fw.img に包んで boot パーティションへ配る。
...
# 8MP SPL スプラッシュのロゴ (単一ソースは u-boot 側の spl_splash_logo.h)。
# ロゴを SPL に埋め込むと ROM のブートイメージ上限 (docs 04-falcon.md ⑤) を
# 超えるため、boot パーティションの logo.bin として配りファイル読みする。
# ヘッダ: "LOGO" + w/h/x/y (各 LE32) + 1bit マスク。
FILESEXTRAPATHS:prepend := "${THISDIR}/../u-boot/files:"
SRC_URI:append:imx8mp-debix = " file://spl_splash_logo.h"

# SPL スプラッシュのロゴは u-boot 側 (SPL) に 1bit マスクで埋め込み済み
# (scripts/gen-splash-raw.py → spl_splash_logo.h、0010 パッチ)。falcon.itb に
...
FALCON_UBOOT_DEPLOY = "u-boot-fslc:do_deploy"
FALCON_UBOOT_DEPLOY:imx8mp-debix = "u-boot-imx:do_deploy"
do_compile[depends] += " \
...
# アドレスは U-Boot パッチ (0001-imx8mm-falcon-mode.patch) の
# FALCON_* 定数、および SPL heap (0x42200000) と衝突しないこと
...
FALCON_BL31 = "bl31-imx8mm.bin"
FALCON_BL31:imx8mp-debix = "bl31-imx8mp.bin"
FALCON_DTB = "imx8mm-xpi.dtb"
FALCON_DTB:imx8mp-debix = "imx8mp-debix.dtb"
FALCON_NODTB = "u-boot-nodtb.bin"
FALCON_NODTB:imx8mp-debix = "imx-boot-tools/u-boot-nodtb.bin-${MACHINE}-sd"
FALCON_UBOOT_DTB = "u-boot-proper.dtb"
FALCON_UBOOT_DTB:imx8mp-debix = "imx-boot-tools/imx8mp-evk.dtb-sd"

do_compile() {
    cp ${DEPLOY_DIR_IMAGE}/Image ${B}/Image
    cp ${DEPLOY_DIR_IMAGE}/${FALCON_BL31} ${B}/bl31.bin
    cp ${DEPLOY_DIR_IMAGE}/${FALCON_NODTB} ${B}/u-boot-nodtb.bin
    cp ${DEPLOY_DIR_IMAGE}/${FALCON_UBOOT_DTB} ${B}/u-boot-proper.dtb
    ...
    # spl_splash_logo.h + 0010 パッチ) で、SPL が fill 直後・eLCDIF RUN 前に
    ...
    # ヘッダ: magic "M4FW" + payload長 + CRC32 + version (各 LE32)。
    # 形式は SPL パッチ 0011-imx8mm-spl-m4-file-read と一致必須。
...
"
    ...
    for slot in a b; do
        ...
        cp ${DEPLOY_DIR_IMAGE}/${FALCON_DTB} ${B}/falcon-$slot.dtb
        ...
        if [ -n "${SPL_SPLASH}" ]; then
            ...
            # /chosen に splash-active を立てる。カーネル側の splash 引き継ぎ
            # パッチ (0004-0009) はこのプロパティで発動する。SPL 実行時セットは
            # 未実装なので、splash ビルド (SPL_SPLASH) の falcon DTB に焼き込む。
            # パッチ側は実ハード状態 (ドメイン ON / チップ生存 / 解像度一致) も
            # 検証してから引き継ぐので、万一 splash 未描画でも安全にフォールバックする。
            fdtput -t s ${B}/falcon-$slot.dtb /chosen splash-active "1"
        fi
...
/dts-v1/;
/ {
	description = "falcon boot slot $slot (ATF + Linux + XPI DTB)";
	#address-cells = <1>;
	images {
		atf {
...
/dts-v1/;
/ {
	description = "falcon fallback: U-Boot proper via ATF";
	#address-cells = <1>;
	images {
		uboot {
...
do_compile:append:imx8mp-debix() {
    # spl_splash_logo.h → logo.bin (SPL がファイル読みする 1bit マスク)
    python3 - <<'PYEOF'
import re, struct
src = open('${WORKDIR}/spl_splash_logo.h').read()
def val(name):
    return int(re.search(rf'#define {name} (\d+)', src).group(1))
w, h, x, y = val('LOGO_W'), val('LOGO_H'), val('LOGO_X'), val('LOGO_Y')
bits = bytes(int(b, 16) for b in re.findall(r'0x[0-9a-fA-F]{2}', src.split('logo_bits')[1]))
open('${B}/logo.bin', 'wb').write(b'LOGO' + struct.pack('<4I', w, h, x, y) + bits)
PYEOF
}
