SUMMARY = "Seeded U-Boot environment image for the A/B boot flow"
DESCRIPTION = "Merges the built U-Boot's default environment (u-boot-initial-env) \
with the A/B variables and produces uboot-env.bin via mkenvimage. The wks \
rawcopies it into the env storage offset, so the very first boot already runs \
the A/B bootcmd without any manual env surgery."
LICENSE = "MIT"
LIC_FILES_CHKSUM = "file://${COMMON_LICENSE_DIR}/MIT;md5=0835ade698e0bcf8506ecda2f7b4f302"

DEPENDS = "u-boot-tools-native"

SRC_URI = "file://ab-env.txt"

inherit deploy nopackages

COMPATIBLE_MACHINE = "(mx8mm-generic-bsp|imx8mp-debix)"
PACKAGE_ARCH = "${MACHINE_ARCH}"

# U-Boot の CONFIG_ENV_SIZE と一致させること (8MM: ab.cfg の 0x2000、
# 8MP: NXP defconfig 既定の 0x4000)
ENV_IMAGE_SIZE = "0x2000"
ENV_IMAGE_SIZE:imx8mp-debix = "0x4000"
# env 2 面化 (U-Boot の CONFIG_SYS_REDUNDAND_ENVIRONMENT と一致させること)。
# 1 のとき mkenvimage -r (CRC の後ろに flags 1B) で作り、同じ面を 2 つ連結して
# 2 面分 (ENV_IMAGE_SIZE × 2) の uboot-env.bin にする。wks は 1 面目のオフセット
# に rawcopy するだけで 2 面目 (直後) も埋まる。両面 flags 同値 → U-Boot は 1 面目を採用。
ENV_REDUNDANT = "0"
ENV_REDUNDANT:imx8mp-debix = "1"

# initial env は u-boot-fslc:do_deploy が置く。ファイル名は
# ${UBOOT_CONFIG} (この machine では sd 固定) が付く。
INITIAL_ENV = "u-boot-fslc-initial-env-sd"
INITIAL_ENV:imx8mp-debix = "u-boot-imx-initial-env-sd"
AB_UBOOT_DEPLOY = "u-boot-fslc:do_deploy"
AB_UBOOT_DEPLOY:imx8mp-debix = "u-boot-imx:do_deploy"
do_compile[depends] += "${AB_UBOOT_DEPLOY}"
# SIT (ROM の secondary image table) は 8MM/8MQ のみ。8MN/8MP は fuse で
# 決まる固定オフセット (未ヒューズ = 4MiB) へフォールバックするため不要
# (meta-kart/wic/imx8mp-emmc-ab.wks のコメント参照)。
MAKE_SIT = "1"
MAKE_SIT:imx8mp-debix = "0"

do_compile() {
    # --- Secondary Image Table (SIT) ---
    # i.MX8M ROM のブートローダ A/B 用。sector 0x41 (33280B) に置かれ、
    # A copy (sector 0x42) が無効なとき ROM が firstSectorNumber (0x1000) +
    # 0x42 = sector 0x1042 の B copy へ自動フォールバックする。
    # 構造 (20B, LE): [8B zero][magic 0x00112233][firstSectorNumber][4B zero]
    # wic の --align は KiB 単位で sector 0x41 (32.5KiB) を直接指せないため、
    # sector 0x40 (32KiB) 起点の 1KiB ブロブ (先頭 512B は零) にして
    # --align 32 で配置する。docs.u-boot.org/en/v2021.07/imx/misc/psb.html
    if [ "${MAKE_SIT}" = "1" ]; then
    dd if=/dev/zero of=${B}/sit.bin bs=512 count=2 2>/dev/null
    # sector 0x41 内 offset 0x08: magic 0x00112233 (LE) / 0x0C: firstSectorNumber 0x1000 (LE)
    # seek=520 = 512 (先頭パディング) + 8 (SIT 内オフセット)。
    # bitbake のシェルパーサは $(( )) を解釈できないためリテラルで書く。
    printf '\063\042\021\000\000\020\000\000' | \
        dd of=${B}/sit.bin bs=1 seek=520 conv=notrunc 2>/dev/null
    test "$(wc -c < ${B}/sit.bin)" = "1024"
    fi

    # --- U-Boot environment ---
    # initial env + A/B 追加分を連結し、重複キーは後勝ちでマージする
    # (mkenvimage 自体は重複を解決しないため)。# 行と空行は除去。
    cat ${DEPLOY_DIR_IMAGE}/${INITIAL_ENV} ${WORKDIR}/ab-env.txt \
      | awk -F= '
          /^[#[:space:]]/ || !/=/ { next }
          {
              key = $1
              sub(/^[^=]*=/, "")
              if (!(key in val)) order[++n] = key
              val[key] = $0
          }
          END { for (i = 1; i <= n; i++) print order[i] "=" val[order[i]] }
        ' > ${B}/uboot-env.txt
    if [ "${ENV_REDUNDANT}" = "1" ]; then
        mkenvimage -r -s ${ENV_IMAGE_SIZE} -o ${B}/uboot-env-copy.bin ${B}/uboot-env.txt
        cat ${B}/uboot-env-copy.bin ${B}/uboot-env-copy.bin > ${B}/uboot-env.bin
    else
        mkenvimage -s ${ENV_IMAGE_SIZE} -o ${B}/uboot-env.bin ${B}/uboot-env.txt
    fi
}

do_deploy() {
    install -m 0644 ${B}/uboot-env.bin ${DEPLOYDIR}/uboot-env.bin
    if [ -f ${B}/sit.bin ]; then
        install -m 0644 ${B}/sit.bin ${DEPLOYDIR}/sit.bin
    fi
    # 検証・デバッグ用にテキストも残す
    install -m 0644 ${B}/uboot-env.txt ${DEPLOYDIR}/uboot-env.txt
}
addtask deploy after do_compile before do_build
