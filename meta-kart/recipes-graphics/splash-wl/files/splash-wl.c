/* splash-wl — SPL スプラッシュと同一の絵 (真っ黒 BG + 白ロゴ) を
 ...
 *
 * ロゴは SPL と同じ 1bit マスク (spl_splash_logo.h、u-boot レシピと共用) を
 * 同じ座標 (LOGO_X/Y、パネル可視域 800x480 の左上原点) に描く。
...
draw(uint32_t *px, int w, int h)
{
	...
	for (row = 0; row < LOGO_H; row++) {
		...
		for (col = 0; col < LOGO_W; col++) {
			int x = LOGO_X + col;
			int bit = row * LOGO_W + col;

			if (x >= w)
				break;
			if (logo_bits[bit >> 3] & (0x80 >> (bit & 7)))
...
main(void)
{
	...
	if (!dpy) {
		fprintf(stderr, "splash-wl: no wayland display\n");
		return 1;
	}
	...
	if (!compositor || !shm || !wm_base) {
		fprintf(stderr, "splash-wl: missing globals\n");
		return 1;
	}
	...
	xdg_toplevel_set_title(toplevel, "splash");
	xdg_toplevel_set_app_id(toplevel, "splash");
	...
	stride = win_w * 4;
	size = stride * win_h;
	fd = memfd_create("splash", 0);
	if (fd < 0 || ftruncate(fd, size) < 0) {
		perror("splash-wl: shm");
		return 1;
	}
	px = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
	if (px == MAP_FAILED) {
		perror("splash-wl: mmap");
		return 1;
	}
	draw(px, win_w, win_h);
